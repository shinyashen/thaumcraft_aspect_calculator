...
- **策略 1（细粒度优先）**：数值最小的签名优先（如 `metallum=1` 优于 `metallum=15`），便于精确微调。同等 (val, num_aspects) 下，多要素覆盖签名优先
- **策略 1.5（覆盖注入）**：将深层候选中的多覆盖签名注入到池中部，确保跨目标签名在扩展循环的 `top_k × 2` 截断范围内
- **策略 2（纯度优先）**：每个数值至少保留一种签名，多侧面要素的签名靠后
- **策略 3（多要素覆盖）**：收集仍未被以上策略收录的多覆盖签名

> **确定性保证**：所有集合迭代均已排序（`sorted()`），同 (val, num_aspects) 组内 `-other_coverage` 降序 + 签名名字母序作为 tiebreaker。相同输入总是产生相同输出。
