...
- **策略 1（细粒度优先）**：数值最小的签名优先（如 `metallum=1` 优于 `metallum=15`），便于精确微调。同等 (val, 纯度) 下，多要素覆盖签名优先
- **策略 2（纯度优先）**：每个数值至少保留一种签名，多侧面要素的签名靠后
- **策略 3（多要素覆盖）**：在需求包含多个目标要素时，收集所有能同时提供多种目标要素的签名
- **优先级重排**：池构建后，按多要素覆盖度降序重排，确保扩展循环尽早尝试最有价值的签名

> **确定性保证**：所有集合迭代均已排序，同 (val, num_aspects) 组内按签名名字母序排列。相同输入总是产生相同输出。
